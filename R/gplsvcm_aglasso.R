...
#' @param X The design matrix (without intercept) of \code{n} by \code{np} where \code{np} is the
#'   number of covariates. Each row is a vector of the covariates for an
#'   observation.
#' @param uvpop The coordinates of population grid points over the domain, default is \code{NULL}.
#' @param U A \code{n} by two matrix where each row is the coordinates of an
#'   observation.
#' @param index The row indexes of the observed data points \code{U} in the population grid points
#'   \code{uvpop}.
...
#' @param criteria A character string specifying the criteria
...
# structure selection
  ...
  ind_l=mfit0$ind_l+1
  ind_nl=mfit0$ind_nl+1
  X=as.matrix(cbind(rep(1,dim(X)[1]),X))
  X_l=as.matrix(X[,ind_l])
  X_nl=as.matrix(X[,ind_nl])
  mfit=gplsvcm_fit(Y=Y,X=X,ind_l=ind_l,ind_nl=ind_nl,U=U, V=V, Tr=Tr,d=d,r=r,lambda=lambda,family=family)
  list(ind_l=ind_l,ind_nl=ind_nl,mfit=mfit)
}
